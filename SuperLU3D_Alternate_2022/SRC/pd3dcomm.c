...
int_t dAllocLlu(int_t nsupers, dLUstruct_t * LUstruct, gridinfo3d_t* grid3d)
{
	...
	{
	    /* code */
	    Lnzval_bc_ptr_fileposition[i] = NULL;
	}	
	#endif

	#ifdef SuperLargeScaleGPUBuffer
	int_t *Lrowind_bc_ptr_ilen = intCalloc_dist(nbc);
	#endif
	#endif
	...
	{
	    /* code */
	    Unzval_br_ptr_fileposition[i] = NULL;
	}
	#endif

	#ifdef SuperLargeScaleGPUBuffer
	int_t *Ufstnz_br_ptr_ilen = intCalloc_dist(nbr);
	#endif
	#endif
                    ...
                    dLUstruct_t* LUstruct,  gridinfo3d_t* grid3d, SCT_t* SCT)
{
    ...
    if (myrow == pr)
	{
	    ...
	    if (usub != NULL)
		{
			...
			if (lenv > MaxBuffer)
			{
				for (int_t i = 0; i < lenv/MaxBuffer + 1; i++)
				{
					int_t minstep = i * MaxBuffer;
					int_t len = MaxBuffer;
					if (minstep + len >= lenv)
					{
						len = lenv - minstep;
					...
					if (len > 0)
					{
						MPI_Send(unzval + minstep, len, MPI_DOUBLE, receiver, k, grid3d->zscp.comm);
					}					
	    ...
	    if (usub != NULL)
		{
			...
			if (lenv > MaxBuffer)
			{
				for (int_t i = 0; i < lenv/MaxBuffer + 1; i++)
				{
					...
					if(len){
						MPI_Recv(Uval_buf + minstep , len, MPI_DOUBLE, sender, k,
			     		grid3d->zscp.comm, &status);
					}
...
{
	#if ( DEBUGlevel>=1 )
		CHECK_MALLOC(grid3d->iam, "Enter dp3dScatter()");
	#endif
    gridinfo_t* grid = &(grid3d->grid2d);
    int_t Pc = grid->npcol;
    int_t Pr = grid->nprow;
	...
	#ifdef SuperLargeScaleGPUBuffer
	LUstruct->Glu_persist->xsup_len = nsupers + 1;
	LUstruct->Glu_persist->supno_len = n;
	#endif

	// #ifdef SuperLargeScaleGPUBuffer
	// MPI_Bcast( &(LUstruct->Glu_persist->xsup_len), 1, mpi_int_t, 0,  grid3d->zscp.comm);
	// MPI_Bcast( &(LUstruct->Glu_persist->supno_len), 1, mpi_int_t, 0,  grid3d->zscp.comm);
	// #endif
	...
	#if ( DEBUGlevel>=1 )
		CHECK_MALLOC(grid3d->iam, "Exit dp3dScatter()");
	#endif
		       ...
		       dLUstruct_t * LUstruct, gridinfo3d_t* grid3d)
{
	...
	#if ( DEBUGlevel>=1 )
		CHECK_MALLOC(grid3d->iam, "Enter dscatter3dUPanels()");
	#endif
    dLocalLU_t *Llu = LUstruct->Llu;
    int_t** Ufstnz_br_ptr = Llu->Ufstnz_br_ptr;
    double** Unzval_br_ptr = Llu->Unzval_br_ptr;
	...
	if (flag) {
		...
		if (lenv > MaxBuffer)
		{
			for (int_t i = 0; i < lenv/MaxBuffer + 1; i++)
			{
				...
				if (len > 0)
				{					
					MPI_Bcast( uval + minstep, len, MPI_DOUBLE, 0,  grid3d->zscp.comm);
				}					
		...
		if (grid3d->zscp.Iam)
			Unzval_br_ptr_ilen[lb]=lenv;

		#ifdef SuperLargeScaleGPUBuffer
		if (grid3d->zscp.Iam)
			Llu->Ufstnz_br_ptr_ilen[lb]=lens;
		#endif
		#endif
	...
	#if ( DEBUGlevel>=1 )
		CHECK_MALLOC(grid3d->iam, "Exit dscatter3dUPanels()");
	#endif
...
{
	#if ( DEBUGlevel>=1 )
		CHECK_MALLOC(grid3d->iam, "Enter dscatter3dLPanels()");
	#endif
	...
	if (mycol == pc)
		...
		if (grid3d->zscp.Iam){
			...
			if (len2 > MaxBuffer)
			{
				for (int_t i = 0; i < len2/MaxBuffer + 1; i++)
				{
					int_t minstep = i * MaxBuffer;
					int_t len = MaxBuffer;
					if (minstep + len >= len2)
					...
					if (len > 0)
					{
						MPI_Bcast( lnzval + minstep, len, MPI_DOUBLE, 0,  grid3d->zscp.comm);
					}					
			...
			if (grid3d->zscp.Iam)
				Lnzval_bc_ptr_ilen[ljb]=len2;

			#ifdef SuperLargeScaleGPUBuffer
			if (grid3d->zscp.Iam)
				Llu->Lrowind_bc_ptr_ilen[ljb] = len1;
			#endif
			#endif
...
{
	#if ( DEBUGlevel>=1 )
		CHECK_MALLOC (grid3d->iam, "Enter dreduceAllAncestors3d()");
	#endif
	...
	#ifdef Torch_stat
	printf("%d: ancsReduce = %e s\n", grid3d->iam, SCT->ancsReduce);
	#endif

	#if ( DEBUGlevel>=1 )
		CHECK_MALLOC (grid3d->iam, "Exit dreduceAllAncestors3d()");
	#endif
