...
#ifdef Torch
int_t dIBcast_LPanel_Lsub
/*broadcasts index array lsub and non-zero value
 array lusup of a newly factored L column to my process row*/
(int_t k, int_t k0, int_t* lsub, gridinfo_t *grid,
 int* msgcnt, MPI_Request *send_req, int **ToSendR, int_t *xsup,
 int tag_ub)
{
    int_t Pc = grid->npcol;
 ...
 int tag_ub, int_t i)
{
    ...
    int length1, length2, index1, index2;
    ...
    if (lsub)
    {
        ...
        if (msgcnt[0]%NUMPART)
        {
            length1 = floor(msgcnt[0]/NUMPART);
            index1 = i * length1;
            if (i == NUMPART - 1)
            {
                length1 = msgcnt[0] - i * length1;
            }            
        }
        else
        {
            length1 = (msgcnt[0])/NUMPART;
            index1 = i * length1;
        }

        if (msgcnt[1]%NUMPART)
        {
            length2 = floor(msgcnt[1]/NUMPART);
            index2 = i * length2;
            if (i == NUMPART - 1)
            {
                length2 = msgcnt[1] - i * length2;
            }            
        }
        else
        {
            length2 = (msgcnt[1])/NUMPART;
            index2 = i * length2;
        }
    }
    else
    ...
    for (int_t pj = 0; pj < Pc; ++pj)
    {
        ...
        {           
            ...
            MPI_Isend (lsub + index1, length1, mpi_int_t, pj,
                       SLU_MPI_TAG (0, k0) /* 0 */ ,
                       scp->comm, &send_req[pj]);
            MPI_Isend (lusup + index2, length2, MPI_DOUBLE, pj,
                       SLU_MPI_TAG (1, k0) /* 1 */ ,
                       scp->comm, &send_req[pj + Pc]);
...
#ifdef Torch
int_t dIBcast_UPanel_Usub
/*asynchronously braodcasts U panel to my process row */
(int_t k, int_t k0, int_t* usub, gridinfo_t *grid,
 int* msgcnt, MPI_Request *send_req_u, int *ToSendD, int tag_ub )
{
...
int_t idIBcast_UPanel
/*asynchronously braodcasts U panel to my process row */
(int_t k, int_t k0, int_t* usub, double* uval, gridinfo_t *grid,
 int* msgcnt, MPI_Request *send_req_u, int *ToSendD, int tag_ub, int_t i )
{
    ...
    if (usub)
    {
        ...
        if (msgcnt[2]%NUMPART)
        {
            length1 = floor(msgcnt[2]/NUMPART);
            index1 = i * length1;
            if (i == NUMPART - 1)
            {
                length1 = msgcnt[2] - i * length1;
            }            
        }
        else
        {
            length1 = (msgcnt[2])/NUMPART;
            index1 = i * length1;
        }

        if (msgcnt[3]%NUMPART)
        {
            length2 = floor(msgcnt[3]/NUMPART);
            index2 = i * length2;
            if (i == NUMPART - 1)
            {
                length2 = msgcnt[3] - i * length2;
            }            
        }
        else
        {
            length2 = (msgcnt[3])/NUMPART;
            index2 = i * length2;
        }
    }
    else
    ...
    if (ToSendD[lk] == YES)
    {
        for (int_t pi = 0; pi < Pr; ++pi)
        {
            if (pi != myrow)
            {
                ...
                MPI_Isend (usub + index1, length1, mpi_int_t, pi,
                           SLU_MPI_TAG (2, k0) /* (4*k0+2)%tag_ub */ ,
                           scp->comm,
                           &send_req_u[pi]);
                MPI_Isend (uval + index2, length2, MPI_DOUBLE,
                           pi, SLU_MPI_TAG (3, k0) /* (4*kk0+3)%tag_ub */ ,
                           scp->comm,
                           &send_req_u[pi + Pr]);
...
int_t dIrecv_LPanel_Lsub
/*it places Irecv call for L panel*/
(int_t k, int_t k0,  int_t* Lsub_buf,
 gridinfo_t *grid, MPI_Request *recv_req, dLocalLU_t *Llu, int tag_ub )
{
    int_t kcol = PCOL (k, grid);
...
#ifdef Torch
int_t dIrecv_UPanel_Usub
/*it places Irecv calls to receive U panels*/
(int_t k, int_t k0, int_t* Usub_buf, dLocalLU_t *Llu,
 gridinfo_t* grid, MPI_Request *recv_req_u, int tag_ub )
{
    int_t krow = PROW (k, grid);
    superlu_scope_t *scp = &grid->cscp;  /* The scope of process column. */
...
int_t idIrecv_LPanel
/*it places Irecv call for L panel*/
(int_t k, int_t k0,  int_t* Lsub_buf, double* Lval_buf,
 gridinfo_t *grid, MPI_Request *recv_req, dLocalLU_t *Llu, int tag_ub, int_t i, int *msgcnt )
{
    int_t kcol = PCOL (k, grid);

    superlu_scope_t *scp = &grid->rscp;  /* The scope of process row. */

    int_t length1, length2, index1, index2;
    if (msgcnt[2]%NUMPART)
    {
        length1 = floor(msgcnt[2]/NUMPART);
        index1 = i * length1;
    ...
    MPI_Irecv (Lsub_buf + index1, length1, mpi_int_t, kcol,
               SLU_MPI_TAG (0, k0) /* 0 */ ,
               scp->comm, &recv_req[0]);
    MPI_Irecv (Lval_buf + index2, length2, MPI_DOUBLE, kcol,
               SLU_MPI_TAG (1, k0) /* 1 */ ,
               scp->comm, &recv_req[1]);
    return 0;
}

int_t idIrecv_UPanel
/*it places Irecv calls to receive U panels*/
(int_t k, int_t k0, int_t* Usub_buf, double* Uval_buf, dLocalLU_t *Llu,
 gridinfo_t* grid, MPI_Request *recv_req_u, int tag_ub, int_t i, int *msgcnt  )
{
    int_t krow = PROW (k, grid);
    superlu_scope_t *scp = &grid->cscp;  /* The scope of process column. */
    ...
    MPI_Irecv (Usub_buf + index1, length1, mpi_int_t, krow,
               SLU_MPI_TAG (2, k0) /* (4*kk0+2)%tag_ub */ ,
               scp->comm, &recv_req_u[0]);
    MPI_Irecv (Uval_buf + index2, length2, MPI_DOUBLE, krow,
               SLU_MPI_TAG (3, k0) /* (4*kk0+3)%tag_ub */ ,
               scp->comm, &recv_req_u[1]);
 ...
 gridinfo_t* grid, MPI_Request *recv_req_u, int tag_ub )
{
    ...
    MPI_Recv (Usub_buf, Llu->bufmax[2], mpi_int_t, krow,
               SLU_MPI_TAG (2, k0) /* (4*kk0+2)%tag_ub */ ,
               scp->comm, &status);
    recv_req_u[0]=MPI_REQUEST_NULL;
    MPI_Recv (Uval_buf, Llu->bufmax[3], MPI_DOUBLE, krow,
               SLU_MPI_TAG (3, k0) /* (4*kk0+3)%tag_ub */ ,
               scp->comm, &status);
    recv_req_u[1]=MPI_REQUEST_NULL;
    return 0;
}
