...
{
    #if ( DEBUGlevel>=1 )
        CHECK_MALLOC (grid3d->iam, "Enter dGatherNRformat_loc3d()");
    #endif
    ...
    #if ( DEBUGlevel>=1 )
        CHECK_MALLOC (grid3d->iam, "Exit dGatherNRformat_loc3d()");
    #endif
...
(equations     3              1      Proc 0 -> Procs {0, 1, 2, 3}
 ...
 after gather) 9              3
	...
	*/
    ...
    {
        int_t length, index;
        int_t len = nrhs * A3d->m_loc;
        if (len%NUMPART)
        {
            length = floor(len/NUMPART);
            index = i * length;
            if (i == NUMPART - 1)
            {
                length = len - i * length;
        ...
        // printf("recv: %d: %ld, %ld\n", grid3d->iam, len, length);
        MPI_Irecv(Btmp + index, length, MPI_DOUBLE,
        ...
        if (grid3d->zscp.Iam == 0) {
            ...
            // printf("Send: %d: %ld, %ld\n", grid3d->iam, len, length);
            MPI_Send(B1 + b_disp[p] + index, length, 
