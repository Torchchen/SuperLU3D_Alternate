    ...
    gridinfo3d_t *grid)
{
    ...
    // MPI_Group superlu_grp_save;
    // int *psaveranks;

    // /* Make a list of the processes in the new communicator. */
    // psaveranks = (int *) SUPERLU_MALLOC(grid->nSave * sizeof(int));
    // psaveranks[0] = 0;
    // psaveranks[1] = 7;

    // /* Create the new group. */
    // MPI_Group_incl( mpi_base_group, grid->nSave, psaveranks, &superlu_grp_save );
    // MPI_Comm_create( Bcomm, superlu_grp_save, &grid->gridsave.comm );
    // #endif
    ...
    // #ifndef SuperLargeScale
    // MPI_Comm_rank( grid->gridsave.comm, &(grid->gridsave.iam));
    // grid->gridsave.grid2d = grid->grid2d;
    // #endif
...
void superlu_gridsaveinit3d(MPI_Comm Bcomm, /* The base communicator upon which
					   the new grid is formed. */
                        int nSave, gridinfo3d_t *grid)
{
...
{
    #if ( DEBUGlevel>=1 )
        CHECK_MALLOC (grid->iam, "Enter superlu_gridexit3d()");
    #endif
    ...
    #if ( DEBUGlevel>=1 )
        CHECK_MALLOC (grid->iam, "Exit superlu_gridexit3d()");
    #endif
}
