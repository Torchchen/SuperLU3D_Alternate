...
#pragma omp parallel for reduction(max :local_max_row_size) private(lk,lsub)
    ...
    #ifndef use_mgpus
    int nstreams = 1;//get_num_cuda_streams (); // default 8
    cudaSetDevice(grid->iam%4);
    #else
    int nstreams;
    cudaGetDeviceCount(&nstreams);
    nstreams *= get_num_cuda_streams (); // default 8
    #endif
    ...
    #else
    int nbDevices,orginDevice,gpuid,nvnumDevices;// nvnumDevices is the num of every device
    nvnumDevices=1;
    cudaGetDeviceCount(&nbDevices);
    // nbDevices=2;
    cudaGetDevice(&orginDevice);
    cublasHandle_t **mhandle;
    mhandle = (cublasHandle_t **) SUPERLU_MALLOC(sizeof(cublasHandle_t*)*nbDevices*nvnumDevices);
    for(int j=0;j<nbDevices*nvnumDevices;j++)
    {
        cudaSetDevice(j%nbDevices);
        mhandle[j] = (cublasHandle_t *) SUPERLU_MALLOC(sizeof(cublasHandle_t)*nstreams);
        for(int i = 0; i < nstreams; i++) mhandle[j][i] = create_handle();
    }    
    #endif
    ...
    int nbDevices=4;
    int *deviceId;
    deviceId=(int*)SUPERLU_MALLOC(sizeof(int)*nbDevices);
    for(int i = 0; i < nbDevices; i++)
    {
        deviceId[i]=i;
    }
    cublasXtHandle_t *handlext;
    handlext = (cublasXtHandle_t *) SUPERLU_MALLOC(sizeof(cublasXtHandle_t)*nstreams);
    for(int i = 0; i < nstreams; i++) 
    {
        handlext[i] = create_handlext();
        cublasXtDeviceSelect(handlext[i],nbDevices,deviceId);
        cublasXtSetCpuRatio(handlext[i],CUBLASXT_GEMM,CUBLASXT_DOUBLE,0);
        cublasXtSetPinningMemMode(handlext[i],CUBLASXT_PINNING_ENABLED);
    }
    printf("use cublasxt\n");
    #endif
    ...
    mstreams = (cudaStream_t **) SUPERLU_MALLOC(sizeof(cudaStream_t*)*nbDevices*nvnumDevices);
    for(int j=0;j<nbDevices*nvnumDevices;j++)
    {
        cudaSetDevice(j%nbDevices);
        mstreams[j] = (cudaStream_t *) SUPERLU_MALLOC(sizeof(cudaStream_t)*nstreams);
        for (int i = 0; i < nstreams; ++i)
            checkCuda( cudaStreamCreate(&mstreams[j][i]) );
    }
    ...
    #else
    double **dA,**dB,**dC;
    dA=(double **)SUPERLU_MALLOC(sizeof(double*)*nbDevices*nvnumDevices);
    dB=(double **)SUPERLU_MALLOC(sizeof(double*)*nbDevices*nvnumDevices);
    dC=(double **)SUPERLU_MALLOC(sizeof(double*)*nbDevices*nvnumDevices);
    #endif
    ...
    for(int i=0;i<nbDevices*nvnumDevices;i++)
    {
        cudaSetDevice(i%nbDevices);
        cudaStat = cudaMalloc( (void**)&dA[i], max_row_size*sp_ienv_dist(3)* sizeof(double));
        if (cudaStat!= cudaSuccess) {
            fprintf(stderr, "!!!! Error in allocating A in the device %ld \n",m*k*sizeof(double) );
            return 1;
        }
        ...
        // size of B should be bigu_size
        cudaStat = cudaMalloc((void**)&dB[i], bigu_size * sizeof(double));
        if (cudaStat!= cudaSuccess) {
            fprintf(stderr, "!!!! Error in allocating B in the device %ld \n",n*k*sizeof(double));
            return 1;
        }

        cudaStat = cudaMalloc((void**)&dC[i], buffer_size * sizeof(double) );
        if (cudaStat!= cudaSuccess) {
            fprintf(stderr, "!!!! Error in allocating C in the device \n" );
            return 1;
        }
...
/*************** end ifdef GPU_ACC ****************/
    ...
    double time1=0,time3=0,time4=0,time6=0,time7=0,time8=0;
    /* ##################################################################
       **** MAIN LOOP ****
       ################################################################## */
...
#include "dSchCompUdt-2Ddynamic.c"
    ...
    printf("other times:%f,%f,%f,%f,%f,%f\n",time1,time3,time4,time6,time7,time8);
    }
#endif
    ...
    {
        cudaSetDevice(i%nbDevices);
        SUPERLU_FREE(mhandle[i]);
        SUPERLU_FREE(mstreams[i]);
        cudaFree( (void*)dA[i] ); /* Sherry added */
        cudaFree( (void*)dB[i] );
        cudaFree( (void*)dC[i] );
    }
    SUPERLU_FREE(dA);
    SUPERLU_FREE(dB);
    SUPERLU_FREE(dC);
    SUPERLU_FREE(mhandle);
    SUPERLU_FREE(mstreams);
    #else
    ...
    #ifdef use_cublasxt
    for(int i = 0; i < nstreams; i++) cublasXtDestroy(handlext[i]);
    #endif
#else
//  #ifdef __INTEL_COMPILER
//    _mm_free (bigU);
