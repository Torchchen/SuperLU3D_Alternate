...
{
    #if ( DEBUGlevel>=1 )
        CHECK_MALLOC (grid->iam, "Enter dblock_gemm_scatterTopLeft()");
    #endif
                                ...
                                )
{
    ...
    #if ( DEBUGlevel>=1 )
        CHECK_MALLOC (grid->iam, "Enter dblock_gemm_scatterTopRight()");
    #endif
    Glu_persist_t *Glu_persist = LUstruct->Glu_persist;
    dLocalLU_t *Llu = LUstruct->Llu;
    int_t* xsup = Glu_persist->xsup;
...
{
    #if ( DEBUGlevel>=1 )
        CHECK_MALLOC (grid->iam, "Enter dblock_gemm_scatterBottomLeft()");
    #endif
    Glu_persist_t *Glu_persist = LUstruct->Glu_persist;
    dLocalLU_t *Llu = LUstruct->Llu;
    int_t* xsup = Glu_persist->xsup;
...
{
    #if ( DEBUGlevel>=1 )
        CHECK_MALLOC (grid->iam, "Enter dblock_gemm_scatterBottomRight()");
    #endif
    Glu_persist_t *Glu_persist = LUstruct->Glu_persist;
    dLocalLU_t *Llu = LUstruct->Llu;
    int_t* xsup = Glu_persist->xsup;
...
#pragma omp critical
    ...
    if(Llu->isSave){
        ...
        if(GetVectorStatus(Llu->isUsed_Unzval_br_ptr[lib][iPart]) == Unused){
            Unzval_br_ptr[lib] = load_Unzval_br_ptr(lib,Llu);
        }
        
    }
    if(GetVectorStatus(Llu->isUsed_Unzval_br_ptr[lib][iPart]) == Unused){
        SetVectorStatus(Llu->isUsed_Unzval_br_ptr[lib][iPart], Used);
    }
    ...
    #ifdef Torch
    #ifdef SuperLargeScale
    if(Llu->isSave && Llu->Unzval_br_ptr_ilen[lib]){
        SetVectorStatus(Llu->isUsed_Unzval_br_ptr[lib][iPart], Changed);
    }
    #endif
    #endif
           ...
           )
{
    ...
    if(Llu->Unzval_br_ptr_ilen[lib]){
        int_t iPart= 0;
        Llu->isUsed_Unzval_br_ptr[lib][iPart]=1;
        // #ifdef SuperLargeScaleGPU
        // Llu->Unzval_RecordMatrix[Llu->ilvl][Llu->topoLvl][lib] = 1;
        // #endif
    }

}

int_t dblock_gemm_scatterBottomRight_predict( int_t lb,  int_t j,
                                     HyP_t* HyP,
                                     dLUstruct_t *LUstruct,
                                     gridinfo_t* grid
                                   )
{
    ...
    dblock_gemm_scatter_predict( lb, j, HyP->Ublock_info_Phi, HyP->Remain_info, xsup, grid
...
#pragma omp critical
    {
        if (ib < jb)
        {
            ...
            if(Llu->Unzval_br_ptr_ilen[lib]){ 
                #ifdef SuperLargeScaleGPU
                if (Llu->isUsed_Unzval_br_ptr_Record[lib] == GPUUnused)
                {                    
                    Llu->iumemory += Llu->Unzval_br_ptr_ilen[lib];
                    Llu->iumemory_increase += Llu->Unzval_br_ptr_ilen[lib];
                }
                if (Llu->isUsed_Unzval_br_ptr_Record[lib] < GPUNeeded)
                {
                    if (Llu->isUsed_Unzval_br_ptr_Record[lib] == GPUUsed)
                    {
                        Llu->iumemory += Llu->Unzval_br_ptr_ilen[lib];
                    }                    
                    Llu->isUsed_Unzval_br_ptr_Record[lib] += GPUNeeded;
                }                               
                #endif
            }
        }
        else
        {
            if(Llu->Lnzval_bc_ptr_ilen[ljb]){
                #ifdef SuperLargeScaleGPU
                if (Llu->isUsed_Lnzval_bc_ptr_Record[ljb] == GPUUnused)
                {
                    Llu->ilmemory += Llu->Lnzval_bc_ptr_ilen[ljb];
                    Llu->ilmemory_increase += Llu->Lnzval_bc_ptr_ilen[ljb];
                }
                if (Llu->isUsed_Lnzval_bc_ptr_Record[ljb] < GPUNeeded)
                {
                    if (Llu->isUsed_Lnzval_bc_ptr_Record[ljb] == GPUUsed)
                    {
                        Llu->ilmemory += Llu->Lnzval_bc_ptr_ilen[ljb];
                    }
                    Llu->isUsed_Lnzval_bc_ptr_Record[ljb] += GPUNeeded;
                }
                #endif
            }
        }
