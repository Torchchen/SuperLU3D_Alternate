...
//#endif
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute1()");
	#endif
	...
	#ifdef Torch
	if ( !(isUsed_Unzval_br_ptr =
				(int_t**)SUPERLU_MALLOC(k * sizeof(int_t*))) )
		ABORT("Malloc fails for isUsed_Unzval_br_ptr[].");
	...
	ABORT("Malloc fails for Unzval_br_ptr_fileposition[].");
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute2()");
	#endif
	...
	for (lb = 0; lb < nrbu; ++lb) {
		...
		Unzval_br_ptr_ilen[lb]=len;

		#ifdef SuperLargeScaleGPUBuffer
		Ufstnz_br_ptr_ilen[lb] = len1+1;
		#endif
		#endif	
		...
		Unzval_br_ptr_ilen[lb]=0;

		#ifdef SuperLargeScaleGPUBuffer
		Ufstnz_br_ptr_ilen[lb] = 0;
		#endif
		#endif
	    }
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute3()");
	#endif
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute4()");
	#endif
	...
	#ifdef Torch
	if ( !(isUsed_Lnzval_bc_ptr = (int_t**)SUPERLU_MALLOC(k * sizeof(int_t*))) )
	    ABORT("Malloc fails for isUsed_Lnzval_bc_ptr[].");
	for(i=0;i<k;i++)
	{
	...
	if ( !(Lnzval_bc_ptr_fileposition =
		(fpos_t **)malloc(k * sizeof(fpos_t*))) )
		ABORT("Malloc fails for Lnzval_bc_ptr_fileposition[].");	
	
	#endif

	#ifdef SuperLargeScaleGPUBuffer
	if ( !(Lrowind_bc_ptr_ilen =intCalloc_dist(k)) )
		ABORT("Malloc fails for Lrowind_bc_ptr_ilen[].");
	Lrowind_bc_ptr_ilen[k - 1] = 0;
	#endif
	
	#ifdef test1220
	if ( !(tmpdouble =
              (double**)SUPERLU_MALLOC(k * sizeof(double*))) )
	    ABORT("Malloc fails for tmpdouble[].");
	#endif
	#endif
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute5()");
	#endif
	...
	for (jb = 0; jb < nsupers; ++jb) { /* for each block column ... */
		...
		if ( nrbl ) { /* Do not ensure the blocks are sorted! */
			...
			if (!(lusup_srt = doubleCalloc_dist(len*nsupc)))
			...
			#ifdef Torch
			Lnzval_bc_ptr_ilen[ljb]=len*nsupc;

			#ifdef SuperLargeScaleGPUBuffer
			Lrowind_bc_ptr_ilen[ljb] = len1;
			#endif
    		#endif
			...
			#ifdef Torch
			Lnzval_bc_ptr_ilen[ljb]=0;

			#ifdef SuperLargeScaleGPUBuffer
			Lrowind_bc_ptr_ilen[ljb] = 0;
			#endif
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute6()");
	#endif
...
/* Count the nnzs per block column */
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute7()");
	#endif
		...
		if(Iactive==1){
			...
			if(rank_cnt>1){
				...
				if(Root==myrow){
					...
					if (rank_cnt!=rank_cnt_ref)
					{
						printf("rank_cnt %d rank_cnt_ref %d\n", rank_cnt, rank_cnt_ref);
					}
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute8()");
	#endif
...
if ( !iam) printf(".. Construct Bcast tree for L: %.2f\t\n", t);
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute10()");
	#endif
...
if ( !iam) printf(".. Construct Reduce tree for L: %.2f\t\n", t);
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute11()");
	#endif
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute12()");
	#endif
...
if ( !iam) printf(".. Construct Bcast tree for U: %.2f\t\n", t);
	...
	#if ( DEBUGlevel>=2 )
		CHECK_MALLOC(iam, "Enter pddistribute13()");
	#endif
...
if ( !iam) printf(".. Construct Reduce tree for U: %.2f\t\n", t);
	...
	#ifdef Torch
	Llu->isUsed_Lnzval_bc_ptr=isUsed_Lnzval_bc_ptr;
	...
	#ifdef SuperLargeScaleGPUBuffer
	Llu->Lrowind_bc_ptr_ilen = Lrowind_bc_ptr_ilen;
	Llu->Ufstnz_br_ptr_ilen = Ufstnz_br_ptr_ilen;
	#endif
	...
	// {
	// 	if(i%10){
	// 		if ( !(tmpdouble[i] = doubleMalloc_dist(10)) )
	// 	    	ABORT("Malloc fails for Unzval_br_ptr[*][].");
	// 	}
	// 	else{
	// 		tmpdouble[i] = NULL;
