...
#pragma omp single
		 ...
		 */
		       ...
		       jjj_global - jjj_st /*number of block columns on GPU.
                ...
                // printf("num_streams_used=%d,ncpu_blks=%d,nbrow=%d,ldu=%d,nstreams=%d,num_blks=%d\n",num_streams_used,ncpu_blks,nbrow,ldu,nstreams,jjj_global - jjj_st);
            } /* pragma omp single */
        ...
        for(int i=0;i<nbDevices*nvnumDevices;i++)
        {
            ...
            cudaMemcpy2DAsync(dA[i], nbrow*sizeof(double),
				  &lusup[luptr+(knsupc-ldu)*nsupr],
				  nsupr*sizeof(double), nbrow*sizeof(double),
				  ldu, cudaMemcpyHostToDevice, mstreams[i][0]);
        }
        cudaSetDevice(orginDevice);
        #endif
        #endif
	    }
...
#pragma omp for schedule( SCHEDULE_STRATEGY )
        ...
        #ifdef use_cublasxt
        #ifdef _OPENMP
        // #pragma omp parallel for private(st,st_col,num_col_stream,tempu,tempv1,stream_id,b_offset,c_offset,B_stream_size,C_stream_size)
        #endif
        #endif
        ...
        { // streams on GPU
            ...
            if ( num_col_stream > 0 ) {		
                ...
                #ifdef use_cublasxt
                cublasCheckErrors(
                    cublasXtDgemm(handlext[stream_id],
                ...
                gpuid=stream_id%(nbDevices*nvnumDevices);
                if(nvnumDevices>1){
                    cudaSetDevice(gpuid%nbDevices);
                }
                else{
                    cudaSetDevice(gpuid);
                }     

                cudaMemcpyAsync(dB[gpuid]+b_offset, tempu+b_offset, B_stream_size,
                            cudaMemcpyHostToDevice, mstreams[gpuid][stream_id]);
                
                cublasCheckErrors(
				  cublasSetStream(mhandle[gpuid][stream_id],
						  mstreams[gpuid][stream_id])
				     );

                cublasCheckErrors(
                    cublasDgemm(mhandle[gpuid][stream_id],
                            CUBLAS_OP_N, CUBLAS_OP_N,
                            nbrow, num_col_stream, ldu,
                                                &alpha, dA[gpuid], nbrow,
                            &dB[gpuid][b_offset], ldu,
                            &beta, &dC[gpuid][c_offset],
                                                nbrow));

                // if(!grid->iam){
                //     double ctemp;
                //     cudaMemcpy(&ctemp,dC[gpuid]+c_offset,sizeof(double),cudaMemcpyDeviceToHost);
                //     printf("stream_id=%d,gpuid=%d,c_offset=%ld\n",stream_id,gpuid,c_offset);
                //     printf("dC[c_offset]=%e\n",ctemp);
                // }
                

                checkCuda( cudaMemcpyAsync(tempv1, dC[gpuid]+c_offset,
                        C_stream_size,
                        cudaMemcpyDeviceToHost,
                        mstreams[gpuid][stream_id]) );

                // if(num_streams_used<=4 && !grid->iam)
                // {
                //     printf("B_stream_size=%ld,b_offset=%ld,C_stream_size=%ld,c_offset=%ld,gpuid=%d,stream_id=%d,full_u_cols[st-1]*nbrow=%d\n",B_stream_size,b_offset,C_stream_size,c_offset,gpuid,stream_id,full_u_cols[st-1]*nbrow);
                ...
                // }
                    ...
                    dgemm_("N", "N", &nbrow, &num_col_stream, &ldu,
...
#pragma omp for schedule(SCHEDULE_STRATEGY) nowait
        ...
        //         //     checkCuda( cudaStreamSynchronize(mstreams[j][i]) );
                ...
                for(i = 0; i < num_streams_used; i++) { /* i is private variable */
                    ...
                    #pragma omp barrier
                    {
                        #pragma omp master
                        {
                            ...
                            #else
                            gpuid=i%(nbDevices*nvnumDevices);
                            if(nvnumDevices>1){
                                cudaSetDevice(gpuid%nbDevices);
                            }
                            else{
                            ...
                            checkCuda(cudaStreamSynchronize (mstreams[gpuid][i]));
                            cudaSetDevice(orginDevice);
                            #endif
                        }
