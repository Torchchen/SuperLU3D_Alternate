...
int
sp_dgemv_dist_test(char *trans, double alpha, SuperMatrix *A, double *x, 
	      int incx, double beta, double *y, int incy)
{
	...
	printf("notran %d\n", notran);
	double sum0 = 0;
	for (int_t i = 0; i < Astore->colptr[1]; i++)
	{
		printf("%e, %e, %e\n", Aval[i], x[Astore->rowind[i]], Aval[i] * x[Astore->rowind[i]]);
		sum0 += (Aval[i] * x[Astore->rowind[i]]);
	}
	printf("%e\n", sum0);
