...
{
    #if ( DEBUGlevel>=1 )
        CHECK_MALLOC (grid->iam, "Enter dSchurComplementSetupGPU()");
    #endif
    ...
    if (mycol == kcol)
    {
        ...
        if(Llu->isSave){
            if(Llu->core_status == OutOfCore){
                ABORT("Error core status: dSchurComplementSetupGPU() should be in core status.\n");
            }            
...
#ifdef Torch
int_t dSchurComplementSetupGPU_predict(
    int_t k, msgs_t* msgs,
    packLUInfo_t* packLUInfo,
    int_t* myIperm, 
    int_t* iperm_c_supno, int_t*perm_c_supno,
    ...
    HyP_t* HyP)
{
    ...
    if (msg0 && msg2)       /* L(:,k) and U(k,:) are not empty. */
    {
        ...
        dRgather_L_predict(k, lsub, lusup,  gEtreeInfo, Glu_persist, grid, HyP, myIperm, iperm_c_supno);
        if (HyP->Lnbrow + HyP->Rnbrow > 0)
        {
            dRgather_U_predict( k, 0, usub, uval, bigU,  gEtreeInfo, Glu_persist, grid, HyP, myIperm, iperm_c_supno, perm_u);
        }/*if(nbrow>0) */
