...
#define NRFRMT
	...
	if (!factored) {
		 ...
		 * on the separator tree computed by ParMETIS.                */
			...
			#ifdef Torch_0320
			save_idata_vector_binary(perm_c, n, 0, perm_c_index + grid3d->iam);
			if (grid3d->iam == 0)
			{
				PrintInt10("perm_c", n, perm_c);
			}
			#endif
		}
	    }
	...
	#ifdef test
	if(grid3d->iam==4 && LUstruct->Llu->isSave){
		
		save_LUstruct_txt(n, grid3d, LUstruct);

		int_t index=100000;
		double *lsub=LUstruct->Llu->Lnzval_bc_ptr[index];
		double *lsub1=load_Lnzval_bc_ptr(index,LUstruct->Llu);
		printf("%ld:%ld,%ld\n",index,LUstruct->Llu->Lnzval_bc_ptr_ilen[index],LUstruct->Llu->Lnzval_bc_ptr_fileposition[index]->__pos);
		for(int_t ii=0; ii<LUstruct->Llu->Lnzval_bc_ptr_ilen[index]; ii++)
		{
			if(abs(lsub[ii]-lsub1[ii])>1e-6 || ii<10){
				printf("%ld,%e,%e\n", ii, lsub[ii], lsub1[ii]);
			}
		}

		double *tmp=doubleMalloc_dist(3);
		tmp[0]=1;
		tmp[1]=2;
		tmp[2]=3;
		...
		if(set_iLnzval_bc_ptr_txt(tmp,index,3,3,LUstruct)){
			lsub=LUstruct->Llu->Lnzval_bc_ptr[index];
			double *lsub2=load_Lnzval_bc_ptr(index,LUstruct);
			printf("%ld:%ld,%ld\n",index,LUstruct->Llu->Lnzval_bc_ptr_ilen[index],LUstruct->Llu->Lnzval_bc_ptr_fileposition[index]->__pos);
			for(int_t ii=0; ii<LUstruct->Llu->Lnzval_bc_ptr_ilen[index]; ii++)
			{
				if(abs(lsub[ii]-lsub2[ii])>1e-6 || ii<10){
					printf("%ld,%e,%e\n", ii, lsub[ii], lsub2[ii]);
				}
			}
		}		
	...
	if(LUstruct->Llu->isSave){
		save_LUstruct_txt(n, grid3d, LUstruct);
		dDestroy_LU1 (n, grid, LUstruct);
		// for (int_t i = 0; i < 3; i++)
		// {
		// 	SUPERLU_FREE(LUstruct->Llu->tmpdouble[i]);
		// }
	...
	// 	/* code */
	// 	if(grid3d->iam==i){
	// 		dPrintLblocks(i, nsupers, grid, LUstruct->Glu_persist, LUstruct->Llu);
	// 		dPrintUblocks(i, nsupers, grid, LUstruct->Glu_persist, LUstruct->Llu);
	// 	}
	// }	
	// while (1)
	// {
