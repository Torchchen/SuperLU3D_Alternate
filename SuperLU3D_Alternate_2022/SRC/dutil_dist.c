		  ...
		  Glu_persist_t *Glu_persist, dLocalLU_t *Llu)
{
	...
	if ( index ) { /* Not an empty column */
		...
		#ifdef SuperLargeScale
		if(Llu->isSave){
			nzval = load_Lnzval_bc_ptr(lb,Llu);
		}
		else{
		...
		#ifdef SuperLargeScale
		if(Llu->isSave && nzval){
            if(SUPERLU_FREE(nzval)){
                ABORT("failed in set_iLnzval_bc_ptr_txt() of dPrintLblocks(). \n");
            }
		}
		#endif
		#endif	
		  ...
		  Glu_persist_t *Glu_persist, dLocalLU_t *Llu)
{
	...
	if ( index ) { /* Not an empty row */
		...
		#ifdef SuperLargeScale
		if(Llu->isSave){
			nzval = load_Unzval_br_ptr(lb,Llu);
		}
		else{
