...
#pragma omp critical
    ...
    if(Llu->isSave){
        ...
        if(Llu->core_status == OutOfCore){
            ABORT("Error core status: dscatter_l() should be in core status.\n");
        }
        if(GetVectorStatus(Llu->isUsed_Lnzval_bc_ptr[ljb][iPart]) == Unused){
            Lnzval_bc_ptr[ljb] = load_Lnzval_bc_ptr(ljb,Llu);
        }
        
    }
    if(GetVectorStatus(Llu->isUsed_Lnzval_bc_ptr[ljb][iPart]) == Unused){
        SetVectorStatus(Llu->isUsed_Lnzval_bc_ptr[ljb][iPart], Used);
    }
...
// #pragma omp simd
    ...
    #ifdef SuperLargeScale
    if(Llu->isSave && Llu->Lnzval_bc_ptr_ilen[ljb]){
        SetVectorStatus(Llu->isUsed_Lnzval_bc_ptr[ljb][iPart], Changed);
    }
    #endif
...
#ifdef test
void
dscatter_l_SuperLargeScale_predict (
           int ib,    /* row block number of source block L(i,k) */
           int ljb,   /* local column block number of dest. block L(i,j) */
           int nsupc, /* number of columns in destination supernode */
           int_t iukp, /* point to destination supernode's index[] */
           ...
           gridinfo_t * grid, dLocalLU_t *Llu)
{
    ...
    if(Llu->Lnzval_bc_ptr_ilen[ljb]){
        int_t iPart=floor((double)nPart*Llu->isUsed_Lnzval_bc_ptr[ljb][nPart]/Llu->isUsed_Lnzval_bc_ptr[ljb][nPart+1]);
        Llu->isUsed_Lnzval_bc_ptr[ljb][iPart]=1;
    }
