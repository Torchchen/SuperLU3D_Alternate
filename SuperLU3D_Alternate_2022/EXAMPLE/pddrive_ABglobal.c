...
int main(int argc, char *argv[])
{
    ...
    dallocateA_dist(n, nnz, &a_global, &asub_global, &xa_global);
    int_t i;
    for(i=0;i<nnz;i++)
    {
        a_global[i]=a[i];
        asub_global[i]=asub[i];
    }
    for (i = 0; i < n+1; i++)
    {
        xa_global[i]=xa[i];
    ...
    dCreate_CompCol_Matrix_dist(&A_global, m, n, nnz, a_global, asub_global, xa_global,
				SLU_NC, SLU_D, SLU_GE);
    ...
    *trans = 'N';
    ...
    load_ddata_vector_txt(b,m,ntimestep,1003);
    #if ( DEBUGlevel>=1 )
        if ( !iam )	printf("read b[%d] = %f\n", m-1, b[m-1]);
    #endif

    double *r;
    if (!(r=doubleMalloc_dist(m*nrhs))) ABORT("Malloc fails for r[]");
    for(i=0;i<m*nrhs;i++)
    {
        r[i]=b[i];
    }
    ...
    if ( !iam ) {
        ...
        int_t incy=1;
        char trans='N';
        double rnrm=dnrm2_(&n,r,&incx);
        sp_dgemv_dist(&trans, alpha, &A_global, b, 1, beta, r, 1);
        double dnrm=dnrm2_(&n,r,&incx);
        printf("res=%e\n",dnrm/rnrm);
    }
