	     ...
	     double **nzval, int_t **rowind, int_t **colptr)
{
    ...
    sprintf(filename,"/home/412-23/collision_model18/data/data-IFMT_IFMT.txt\0",index,ntimestep);
    
    if(!access(filename,0)){
        FILE *fp=fopen(filename,"r");
        sleep(5);
        i=0;
        while(fscanf(filename,IFMT,matrixlen[i]))
        {
            i++;
        }
        *nonz=matrixlen[0];
...
static void load_ddata_markders_vector_txt(double *data,int_t n,int_t ntimestep,int_t index)
{
    ...
    for(j=0;j<n/minstep+1;j++)
    {
        maxstep=(j+1)*minstep;
        if(maxstep>=n){
            maxstep=n;
        }
        int slen=strlen("/home/412-23/collision_model18/data/data-\0")+strlen("_\0")+strlen(".txt\0")++strlen("-\0")-4;
        slen=slen+(ntimestep==0?1:(int)log(ntimestep)+1);
        slen=slen+(index==0?1:(int)log(index)+1);
        slen=slen+(j==0?1:(int)log(j)+1);
        char *filename=(char*)malloc(slen+1);
        sprintf(filename,"/home/412-23/collision_model18/data/data-IFMT_IFMT-IFMT.txt\0",index,ntimestep,j);
        ...
        if(access(filename,0)){
            ...
            i=(j-1)*minstep;
            while (fscanf(filename,IFMT,data[i]))
            {
                i++;
            }
        }
