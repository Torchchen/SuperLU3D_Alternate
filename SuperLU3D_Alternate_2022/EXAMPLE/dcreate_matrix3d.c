                     ...
                     FILE *fp, gridinfo3d_t *grid3d)
{
    ...
    *trans = 'N';
    ...
    load_ddata_vector_txt(b_global,m,matrixlen[2],1003);
    #if ( DEBUGlevel>=1 )
        if ( !iam )	printf("read b_global[%d] = %f\n", m-1, b_global[m-1]);
    #endif
...
#if ( DEBUGlevel>=1 )
    printf("sizeof(NRforamt_loc) %lu\n", sizeof(NRformat_loc));
    CHECK_MALLOC(iam, "Exit dcreate_matrix3d()");
#endif
    return 0;
}
#else
...
int dcreate_matrix_postfix3d2(SuperMatrix *A, SuperMatrix *GA, int nrhs, double **rhs,
                           int *ldb, int *ldx, char * postfix, gridinfo3d_t *grid3d, int_t *ntimestep)
{
    ...
    if ( !iam )
    {
        ...
        {
            /* Read the matrix stored on disk in Harwell-Boeing format. */
            dreadhb_dist2(iam, &m, &n, &nnz, &nzval, &rowind, &colptr, &b_global, ntimestep);
        }        
        else
        {
            ABORT("File format not known");
        ...
        MPI_Bcast( rowind, nnz, mpi_int_t,  0, grid3d->comm );
        MPI_Bcast( colptr, n + 1, mpi_int_t,  0, grid3d->comm );
        MPI_Bcast( b_global, n, MPI_DOUBLE,  0, grid3d->comm );
    }
    else
    {
        /* Receive matrix A from PE 0. */
...
int dcreate_matrix_postfix3d3(SuperMatrix *A, SuperMatrix *GA, int nrhs, double **rhs,
                           int *ldb, int *ldx, char * postfix, gridinfo3d_t *grid3d, int_t *ntimestep)
{
    int_t    *rowind, *colptr;   /* global */
    double   *nzval;             /* global */
    ...
    if ( !iam )
    {
        ...
        {
            /* Read the matrix stored on disk in Harwell-Boeing format. */
            dreadhb_dist3(iam, &m, &n, &nnz, &nzval, &b_global, ntimestep);
        }        
        else
        {
            ABORT("File format not known");
    ...
    if(GAstore->nnz==nnz){
        for (i = 0; i < nnz; ++i)
        {
            ((double *)GAstore->nzval)[i] = nzval[i];
        }
    }   
    else{
        ABORT("nnz has changed.\n");
    } 
